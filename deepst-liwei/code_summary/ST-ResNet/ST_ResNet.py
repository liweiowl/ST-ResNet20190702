...
from keras.layers import add, LSTM, Flatten
...
def add_lstm(data_in, nb_flow=2, map_height=16, map_width=8):
    # data_out = data_in
    main_output1 = Reshape((nb_flow * map_height, map_width))(data_in)
...
def stresnet_lstm(c_conf=(3, 2, 32, 32), p_conf=(3, 2, 32, 32), t_conf=(3, 2, 32, 32), external_dim=8,
                  nb_residual_unit=3):
    '''
    C - Temporal Closeness
    P - Period
    ...
    add a lstm layer in the end to extract the temporal properities
    '''
    ...
    if len(outputs) == 1:
        ...
        new_outputs = []
        for output in outputs:
            tmp_out = iLayer()(output)
            tmp_out = add_lstm(tmp_out)
            new_outputs.append(tmp_out)
            # new_outputs = add_lstm(new_outputs)
        #        main_output = merge(new_outputs, mode='sum')
        main_output = add(new_outputs)

    # main_output = add_lstm(main_output)
